...
test('When pubkeys are set, and outputs are set, then correct set of nodes is built.', async t => {
	...
	accountTree.SetInputPubKeysHex(inputPubKeys);

	inputPubKeys.forEach((pubKeyHex) => {
		let accountNode = accountTree.GetLeafAccountNodeForPublicKeyHex(pubKeyHex);
		accountNode.AddIncomingLeafSendBlock({}, '20');
	});
...
let getTestObjects = () => {
	...
	mockBlockBuilder.GetUnsignedSendBlock = (() => {
		return {
			hash: 'asdf'
		};
	});
